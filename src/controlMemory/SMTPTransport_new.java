  ...
  extends Transport
{
  ...
  protected SMTPTransport(Session session, URLName urlname, String name, boolean isSSL)
  {
    ...
    this.noauthdebug = ((this.debug) && (!PropUtil.getBooleanSessionProperty(session, "mail.debug.auth", false)));
    if (urlname != null) {
    ...
    SMTPTransport.Authenticator[] a = { new SMTPTransport.LoginAuthenticator(this), new SMTPTransport.PlainAuthenticator(this), new SMTPTransport.DigestMD5Authenticator(this), new SMTPTransport.NtlmAuthenticator(this) };
    ...
    throws MessagingException
  {
    ...
    while (st.hasMoreTokens())
    {
      ...
      else
      {
        ...
        {
          SMTPTransport.Authenticator a = (SMTPTransport.Authenticator)this.authenticators.get(m);
          if (a == null)
          {
            if (this.debug) {
    ...
    throws MessagingException
  {
    if (this.saslAuthenticator == null) {
      try
      {
        ...
        Constructor c = sac.getConstructor(new Class[] { SMTPTransport.class, String.class, Properties.class, Boolean.TYPE, PrintStream.class, String.class });
        
        this.saslAuthenticator = ((SaslAuthenticator)c.newInstance(new Object[] { this, this.name, this.session.getProperties(), this.debug ? Boolean.TRUE : Boolean.FALSE, this.out, this.host }));
      }
      catch (Exception ex)
      {
        if (this.debug) {
    ...
    {
      List v = new ArrayList(allowed.length);
    ...
    try
    {
      if (this.noauthdebug)
      {
        this.out.println("DEBUG SMTP: SASL AUTH command trace suppressed");
        suspendTracing();
      }
    ...
    throws MessagingException, SendFailedException
  {
    ...
    catch (MessagingException mex)
    {
      if (this.debug)
      {
        this.out.println("DEBUG SMTP: MessagingException while sending");
        mex.printStackTrace(this.out);
      }
      if ((mex.getNextException() instanceof IOException))
      {
        if (this.debug) {
          this.out.println("DEBUG SMTP: nested IOException, closing");
        }
        try
        {
          closeConnection();
    ...
    catch (IOException ex)
    {
      if (this.debug)
      {
        this.out.println("DEBUG SMTP: IOException while sending, closing");
  ...
  private boolean convertTo8Bit(MimePart part)
  {
    ...
    catch (IOException ioex) {}catch (MessagingException mex) {}
    return changed;
  }
    ...
    throws IOException
  {
    ...
    this.traceInput = new TraceInputStream(this.serverSocket.getInputStream(), out);
    this.traceInput.setTrace(this.debug);
    this.traceInput.setQuote(quote);
    
    this.traceOutput = new TraceOutputStream(this.serverSocket.getOutputStream(), out);
